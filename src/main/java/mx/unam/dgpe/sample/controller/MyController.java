...
public class MyController extends AbstractVerticle {
    ...
    public void start(Future<Void> fut) {
        ...
        router.get("/calculadora/primero").handler(this::primero);
        router.get("/calculadora/suma").handler(this::suma);
        router.get("/calculadora/resta").handler(this::resta);
        router.get("/calculadora/mult").handler(this::mult);
        router.get("/calculadora/div").handler(this::div);
    ...
    private void suma(RoutingContext routingContext) {
        ...
        String jsonResponse = procesa(operacion,numA,numB,numC);
        response.setStatusCode(200).
        putHeader("content-type", "application/json; charset=utf-8").
        end(jsonResponse);
    }
