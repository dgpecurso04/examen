...
public class TestMyController extends AbstractVerticle {
	...
	String result = sendGet("http://localhost:8081/calculadora/suma?numA=2&numB=3");
        assertTrue("Este es un mensaje", result.length() >1);
        logger.info(result);

	result = sendGet("http://localhost:8081/calculadora/resta?numA=2&numB=3");
        assertTrue("Este es un mensaje", result.length() >1);
        logger.info(result);
	
	result = sendGet("http://localhost:8081/calculadora/mult?numA=2&numB=3");
        assertTrue("Este es un mensaje", result.length() >1);
        logger.info(result);

	result = sendGet("http://localhost:8081/calculadora/div?numA=2&numB=3");
        assertTrue("Este es un mensaje", result.length() >1);
        logger.info(result);
